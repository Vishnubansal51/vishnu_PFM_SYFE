...
//          // Create unique filenames using userId and current timestamp
...
//         // Filter transactions by year only
//         const filtered = transactions.filter(transaction => {
//             const date = new Date(transaction.date);
...
//         // Calculate total income and expenses based on the 'type' column
//         const totalIncome = filtered
//             .filter(t => t.type === 'income')
//             .reduce((sum, t) => sum + t.amount, 0);
...
//         // Group transactions by category with type-based breakdown
//         const categoryBreakdown = filtered.reduce((acc, t) => {
//             const categoryName = t.Category ? t.Category.name : 'Uncategorized';
//             if (!acc[categoryName]) {
//                 acc[categoryName] = { income: 0, expense: 0 };
...
//         // Respond with the yearly report
//         res.json({
//             totalIncome,
//             totalExpense,
//             categoryBreakdown,
//             pieChartPath, // Base64-encoded URL
//             barGraphPath, // Base64-encoded URL
//             transactions: filtered,
//         });
//     } catch (error) {
//         res.status(500).json({ message: 'Error generating report', error });
